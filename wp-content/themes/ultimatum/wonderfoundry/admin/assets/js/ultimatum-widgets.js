...
(function($) {
	...
	wpWidgets = {
		...
		saveOrder : function( sidebarId ) {
			var data = {
				action: 'ultimatum-widgets-order',
...
function slideOpts(selector){
	var div = jQuery(selector).siblings('div');
	var items  = jQuery('div.options',div);
	items.hide();
	jQuery("."+jQuery(selector).val(),div).show();
}
function menuOpts(selector){
	var div = jQuery(selector).siblings('div');
...
function ult_meta_div(selector){
    var div =  jQuery(selector).closest('div');
    var fieldset = jQuery('fieldset',div);
    if(jQuery(selector).val()!='false'){
...
function ult_loop_image_div(selector){
    var div =  jQuery(selector).closest('div');
    var fieldset = jQuery('fieldset',div);
    if(jQuery(selector).val()!='nimage' && jQuery(selector).val()!='1-col-n' && jQuery(selector).val()!='3-col-n' && jQuery(selector).val()!='4-col-n' && jQuery(selector).val()!='2-col-n' ){
        jQuery(fieldset).show();
    } else {
        jQuery(fieldset).hide();
    }
