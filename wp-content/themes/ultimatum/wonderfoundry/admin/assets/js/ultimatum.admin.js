...
jQuery(document).ready(function() {
	jQuery(window).scroll(function() {    
		...
		jQuery("#ult-nav").removeClass("navbar-fixed-top");
	    // get the amount the window has scrolled
	    var scroll = jQuery(window).scrollTop();
	    // add the 'active' class to the correct li based on the scroll amount
	    if (scroll >= 100) {
	    	jQuery("#ult-nav").addClass("navbar-fixed-top");
	    }
	   
	});
	jQuery('#setting-submit').click(function(){
		jQuery('#ult-setting-form').submit();
	});
	...
	if (jQuery(".ult-color-field")[0]){
		jQuery('.ult-color-field').wpColorPicker(myOptions);
	}
	// Upload Buttons
	jQuery('.option-upload-button').each(function(){
		var clickedObject = jQuery(this);
		var target = jQuery(this).attr('data-id');
		jQuery(clickedObject).live('click', function( event ){
			event.preventDefault();
    ...
    jQuery("select.ult-select-2").select2({width:'copy',allowClear:true});
	jQuery('a#reset-form').click(function(){
        var where_to= confirm("Do you really want to reset the section? This action is irreversible");
        if (where_to== true){
            resetForm(jQuery('#ult-setting-form'));
            jQuery('#ult-setting-form').submit();
        }
    });//ult-setting-form
});
